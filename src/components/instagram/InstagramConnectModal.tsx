...
export const InstagramConnectModal: React.FC<InstagramConnectModalProps> = ({ isOpen, onClose }) => {
  ...
  const currentMetaConfig = metaApiService.getMetaConfig();
  ...
  const handleSaveMetaConfig = () => {
    if (!inputAppId.trim()) {
      addToast('Informe o Meta App ID.', 'warning');
      return;
    }
    metaApiService.saveMetaConfig({ appId: inputAppId.trim(), appSecret: inputAppSecret.trim() });
    addToast('✅ Configurações do Meta App salvas com sucesso no aplicativo!', 'success');
  };

  const handleStartMetaOAuth = () => {
    if (metaApiService.isMetaConfigured()) {
      const url = metaApiService.getMetaAuthUrl();
      if (url && url !== '#') {
    ...
    addToast('Informe o Meta App ID acima para conectar via verificação real.', 'warning');
  };
          ...
          <div className="space-y-4">
            <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-3">
              ...
              <div className="flex items-center justify-between text-xs pt-2 border-t border-slate-900">
                <span className="text-slate-400 font-medium">Status da Meta API:</span>
            ...
            {/* Inserção direta do Meta App ID / Secret no aplicativo */}
            <div className="p-4 rounded-2xl bg-slate-950/80 border border-slate-800 space-y-3">
              <div className="flex items-center justify-between">
                <h5 className="text-xs font-bold text-slate-200">Configuração Direta do Meta App ID</h5>
                <button
                  type="button"
                ...
                >
                  ...
                  <span>{showTutorial ? 'Ocultar Guia' : 'Como obter?'}</span>
                </button>
              </div>

              <div className="grid grid-cols-1 gap-2">
                <div>
                  <label className="block text-[11px] font-medium text-slate-400 mb-1">Meta App ID</label>
                  <input
                    type="text"
                    value={inputAppId}
                    onChange={(e) => setInputAppId(e.target.value)}
                    placeholder="Ex: 123456789012345"
                    className="w-full px-3 py-2 rounded-xl bg-slate-900 border border-slate-800 text-xs text-white placeholder-slate-600 focus:border-indigo-500 focus:outline-none transition-all font-mono"
                  />
                </div>
                <div>
                  <label className="block text-[11px] font-medium text-slate-400 mb-1">Meta App Secret (Chave Secreta)</label>
                  <input
                    type="password"
              ...
              <button
                ...
                className="w-full py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold border border-slate-700 transition-all flex items-center justify-center gap-1.5"
              >
                <span>Salvar Configuração da Meta API no App</span>
              </button>
            </div>
